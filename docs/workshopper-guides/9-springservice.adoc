...
Prerequisites
    ...
    The example application created using the Fabric8 Launcher tool on a Minishift or CDK. For more information, see Creating the Secured example application using Fabric8 Launcher.
...
Clone the project from GitHub. You can do this in your current workspace or you can create a new workspace with a spring stack.

    https://github.com/snowdrop/secured-example.git
...
Lets request an access token with curl, the credentials, and <SSO_AUTH_SERVER_URL> and extract the token from the response with the jq command:
